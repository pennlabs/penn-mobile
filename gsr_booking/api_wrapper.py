...
class LibCalWrapper():
    ...
    def get_token(self):
        ...
        if self.expiration > timezone.localtime():
            return
        body = {"client_id": settings.LIBCAL_ID, "client_secret": settings.LIBCAL_SECRET, "grant_type": "client_credentials"}
        response = requests.post("{}/1.1/oauth/token".format(API_URL), body).json()

        if "error" in response:
            raise APIError("LibCal Auth Failed: {}, {}".format(response["error"], response.get("error_description")))
