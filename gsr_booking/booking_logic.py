...
class BookingLogic:
            ...
            for member in members:
                ...
                if success:
                    ...
                    if not key in bookings:
                        bookings[key] = []
                    new_bookings = self.split_booking(
                    ...
                    next_start = next_end
                    next_end = min(
                        END_DATE, next_start + datetime.timedelta(hours=self.MAX_SLOT_HRS)
                    )
                elif self.is_fatal_error(error):
                    result_json = self.construct_bookings_json_obj(
    ...
    def construct_bookings_json_obj(
        self, succesful_bookings, lid, room, end, next_start, next_end, error
    ):
            ...
            if r.status_code == 200:
                ...
                for reservation in reservations:
                    from_date = datetime.datetime.strptime(
                        reservation["fromDate"], self.DATE_FORMAT_STR
                    )
                    to_date = datetime.datetime.strptime(
                        reservation["toDate"], self.DATE_FORMAT_STR
                    )
