...
class DiningAPIWrapper:
    ...
    def _build_dining_item(self, item_id, item_info):
        ...
        return DiningItem(
            ...
            allergens=", ".join(item_info["cor_icon"].values()) if item_info["cor_icon"] else "",
            **icon_flags,
            nutrition_info=json.dumps(
                {
                    detail["label"]: f"{detail['value']}{detail['unit']}"
                    for detail in item_info["nutrition_details"].values()
                }
            ),
        )
